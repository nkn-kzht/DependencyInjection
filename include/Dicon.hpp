...
namespace cppdicon {
template<> std::unique_ptr<::IClient> Container::ComponentManager::create(const std::string& name) const {
  std::unique_ptr<::IClient> returnInstUp;
  if ("client" == name) {
    std::unique_ptr<::ClientImpl> instUp(new ::ClientImpl(_container.create<ISupplier>("supplier_hello")));
...
template<> std::weak_ptr<::IClient> Container::SingletonComponentManager::getInstance(const std::string& name) const {
  std::shared_ptr<::IClient> returnInstWp;
  if ("client" == name) {
    static std::shared_ptr<::ClientImpl> instSp;
    if (!instSp) {
      instSp = std::make_shared<::ClientImpl>(_container.create<ISupplier>("supplier_bye"));
    }
    returnInstWp = instSp;
  } else {
    static std::shared_ptr<::ClientImpl> instSp;
...
template<> std::unique_ptr<::ISupplier> Container::ComponentManager::create(const std::string& name) const {
...
template<> std::weak_ptr<::ISupplier> Container::SingletonComponentManager::getInstance(const std::string& name) const {
...
class Dicon {
 ...
 private:
  ...
  Dicon& operator=(const Dicon& obj) = delete;
  Dicon& operator=(Dicon&& obj) = delete;
